...
func _get_node_y_positions(number_of_nodes: int) -> Array:
	...
	match number_of_nodes:
		1 : y_positions = [150]
		2 : y_positions = [125, 175]
		3 : y_positions = [100, 150, 200]
		4 : y_positions = [75, 125,175,225]
	return y_positions
			...
			if previous_column:
				for previous_node in previous_column:
					if len(previous_column) == 1 or len(column) == 1:
						...
						if node.y == 125 and previous_node.y <= 150:
							_spawn_line(node, previous_node)
						elif node.y == 175 and previous_node.y >= 150:
							_spawn_line(node, previous_node)
					elif len(column) == 3:
						...
						if len(previous_column) == 2 and node.y < 150 and previous_node.y < 150:
							_spawn_line(node, previous_node)
						elif len(previous_column) == 2 and node.y > 150 and previous_node.y > 150:
							_spawn_line(node, previous_node)
						elif node.y == previous_node.y:
							_spawn_line(node, previous_node)
						elif len(previous_column) == 3 and node.y == 75 and previous_node.y < 150:
							_spawn_line(node, previous_node)
						elif len(previous_column) == 3 and node.y == 225 and previous_node.y > 150:
							_spawn_line(node, previous_node)
